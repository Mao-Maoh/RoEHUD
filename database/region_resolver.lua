...
function resolver.get_region_id(zone_id)
    if not zone_id or zone_id == 0 then return nil end
    for region_id, zone_set in pairs(region_zones.map) do
        if zone_set:contains(zone_id) then
            return region_id
        end
    end
    return nil
...
function resolver.get_region_name_ja(zone_id)
    local region_id = resolver.get_region_id(zone_id)
    if region_id then
        local rg = res.regions[region_id]
        if rg and rg.ja then return rg.ja end
    end
